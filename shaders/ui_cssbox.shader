...
{ 
	...
	CreateTexture2DWithoutSampler( g_tColor ) 	< Attribute( "Texture" ); SrgbRead( false ); >;
	...
	PS_OUTPUT MainPs( PS_INPUT i )
	{
		...
		if ( D_BACKGROUND_IMAGE == 1 )
		{
			...
			if ( BgRepeat == 0 )
				vImage = AttributeTex2DS( g_tColor, g_sRepeatSampler, vUV );
			else if ( BgRepeat == 1 )
				vImage = AttributeTex2DS( g_tColor, g_sRepeatXSampler, vUV );
			else if ( BgRepeat == 2 )
				vImage = AttributeTex2DS( g_tColor, g_sRepeatYSampler, vUV );
			else
				vImage = AttributeTex2DS( g_tColor, g_sClampSampler, vUV );
